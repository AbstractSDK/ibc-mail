...
use abstract_sdk::{AccountVerification, IbcInterface, ModuleInterface, ModuleRegistryInterface};
...
use abstract_std::objects::module::{ModuleInfo, ModuleVersion};
use cosmwasm_std::{CosmosMsg, Deps, DepsMut, Empty, Env, MessageInfo, to_json_binary, wasm_execute};

use ibcmail::{IBCMAIL_CLIENT_ID, Message, Header, Recipient, Route};
...
pub(crate) fn route_msg(deps: DepsMut, msg: Message, header: Header, app: &ServerAdapter) -> ServerResult<CosmosMsg> {
            ...
            match header.route {
                ...
                AccountTrace::Remote(ref chains) => {
                    ...
                    if header.current_hop == chains.len() as u32 {
                        return route_to_local_account(deps.as_ref(), msg.clone(), account_id, header, app)
                    }
                    // TODO verify that the chain is a valid chain

                    let current_module_info = ModuleInfo::from_id(app.module_id(), app.version().into())?;

                    let dest_chain = chains.get(header.current_hop as usize).ok_or(ServerError::InvalidRoute { route: header.route.clone(), hop: header.current_hop })?.to_string();
                    ...
                    let ibc_client_addr = app.module_registry(deps.as_ref())?.query_module(ModuleInfo::from_id_latest(IBC_CLIENT)?)?.reference.unwrap_native()?;
...
fn route_to_local_account(deps: Deps, msg: Message, account_id: &AccountId, header: Header, app: &ServerAdapter) -> ServerResult<CosmosMsg> {
